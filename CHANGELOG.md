...
v2.2.1
* Fix: **ESP-NOW pairing failed on some pendants — no discovery packet was ever sent.** `init_system()` allocated a 240x240 8-bit offscreen canvas (57,600 bytes) at boot on every build. Only `refreshDisplay()` ever puts that buffer on screen, and it is called exclusively from the old Scene classes — the new UI draws to the display directly and through the shared panel sprites, so under `USE_NEW_UI` the canvas was allocated, never drawn, and never freed. Together with the panel scratch it left the heap small and badly fragmented: measured on a resistive pendant at 45 KB free with a **largest free block of 564 bytes**, at which point `esp_now_add_peer()` cannot allocate. The beacon then silently skipped its broadcast, so the pairing screen sat on "Waiting..." indefinitely with nothing on the wire. The canvas is now allocated only for old-UI builds. Capacitive boards had slightly more headroom and usually stayed just above the cliff, which is why this looked like a timing or board-specific fault.
* Fix: **resistive CYD pendants shut down about 5 seconds after boot** ("POWERING OFF"), repeatedly, and sent an unwanted soft reset to the controller each time. The three physical-button pins were assigned but never configured on the resistive board — the capacitive init pulls them up, the resistive one did not. That matters on this variant specifically: the resistive CYD has no connector for those pins, so the RGB LED stays in circuit and users solder buttons straight onto its legs. Left as a bare input, GPIO4 (red) floats at roughly Vcc minus the LED's forward drop — about 1.3 V, inside the ESP32's indeterminate band — and reads LOW more often than not. The button handler reads that as "red held", which fires `Reset` + `$X` at the controller immediately and the red long-press power-off 5 s later. The pins are now pulled up, so an unwired (or partly wired) board reads them as inert.
* Fix: **GPIO4 is left under RTC control after a deep sleep.** It is the ext0 wake source, so `deep_sleep()` hands the pad to the RTC controller and nothing ever hands it back. It is now released before the buttons are configured, so the fix above holds on a wake-from-sleep boot and not only a cold one.
* Fix: **the controller config burst was sent three times** in the seconds after an ESP-NOW link came up. Two separate places posted the connect event — the parser's receive-timing edge and the radio link-up edge added in v2.2.0 — on top of the query burst `onStateChange()` already issues. The parser edge is now UART-only; on ESP-NOW the radio link-up edge is the single trigger.
* Fix: **heavy panel flicker after connecting.** Six different places post a screen-refresh event — every status report, state changes, several parser callbacks and the ESP-NOW link tick — and each one forced an immediate repaint. They arrive in bursts, most visibly right after pairing, driving the redraw far past the display's useful rate. Refreshes are now coalesced with an 80 ms floor and the existing 100 ms periodic tick renders the latest state; these events carry no data, so nothing is lost.
* Diagnostics: the boot log now reports the **largest free heap block** alongside free heap, and a discovery beacon the radio refuses is logged instead of being skipped silently. Free heap alone hid the fault above completely — 45 KB free looks healthy until you see the largest block is 564 bytes. A `cyd_new_ui_combined_debug` build environment was added for this (the shipped build compiles all debug output out, which is why the serial monitor is silent on it).
* Note: the button and RTC fixes are inside the resistive board init — capacitive pendants are untouched. Resistive boards have no Li-Ion PMIC, so they still show no battery indicator; that is by design, not a missing feature.

**2026-08-15**

v2.2.0
* **The WiFi transport has been removed.** It was unreliable in practice — the CYD's small antenna plus FluidNC serving its WebUI and the pendant WebSocket on the same port 80 made it the least dependable way to talk to a controller. **If you rely on WiFi, stay on v2.1.11**, which remains installable from the Releases page. The pendant now offers **UART** (wired, unchanged) and **ESP-NOW** (new).
* Add: **ESP-NOW transport.** A direct, encrypted peer-to-peer radio link to the controller — no access point, no DHCP, no TCP. Requires **FluidNC v4.0.4 or newer**, which ships ESP-NOW in the stock release (no custom controller build needed). Pairing uses P-256 ECDH so each pendant/controller pair gets its own key. Up to **5 machines** can be paired and switched between from the new Machines screen.
  * **Feature parity with UART.** ESP-NOW is a byte-stream transport exactly like UART, so probing, jogging, homing, overrides, SD job streaming, the DRO and macros all work identically — there is no ESP-NOW-specific behaviour to learn.
* Add: **Connection screen** (replaces WiFi Setup). Pick UART or ESP-NOW, see live link state, machine name, peer MAC/channel, signal strength and the receive-path counters (**RX BYTES / LOST / DROP**). Changing transport **reboots the pendant automatically** after a 5-second countdown — the transport is only read at boot, so the restart is required rather than optional. Tapping the other transport during the countdown restarts the timer, so a mis-tap costs nothing.
* Add: **ESP-NOW Pairing screen.** The pairing action happens on the *controller* (`$espnow/pair`, or WebUI > Settings > ESP-NOW), so the screen leads with the steps and ticks them off as they complete. Reached from **Connection > Pair New** — pairing starts when you're ready for it, not automatically at boot. The window times out after 90 s with Retry/Cancel rather than waiting indefinitely.
* Add: **Machines screen.** Lists paired controllers with signal and an ACTIVE marker; tap to switch, Forget (with confirmation) to remove.
* **Change: macros must now be in `macrocfg.json`.** The WebUI v3 `preferences.json` format is **no longer supported** — put your macros in `macrocfg.json` (WebUI v2 format) on the controller. v2.1.x read `preferences.json` first and fell back to `macrocfg.json`; the pendant now uses `macrocfg.json` only.
* Fix: **Macros and the SD card listing never loaded over ESP-NOW** ("loading", then "couldn't load"). Two independent faults, both invisible to the packet-loss counters:
  * FluidNC enables **200 ms auto-reporting** on an ESP-NOW channel as soon as a peer pairs, so ~5 status reports a second are interleaved into the same byte stream as the reply. The pendant streamed *every* byte into the JSON parser while a request was in flight, which fed those `<Idle|MPos:...>` lines into the document — killing the parse *and* stopping the reports ever reaching the line parser, which is why the DRO went stale at the same time. Incoming data is now demultiplexed **per line**: `<` and `[` lines go to the normal parser, everything else is reply payload. The decision is made on each line's first byte with nothing buffered.
  * Fragment reassembly had **one slot**. Every outgoing message carries its own sequence number, so a multi-line reply is a burst of separate messages, and ESP-NOW gives no cross-message ordering. A fragment of a new message silently destroyed the one in progress, so any multi-fragment message that got overlapped was lost — while short single-fragment ones (the version string, status reports) always survived. There are now four slots, and each message is emitted as soon as it completes. (Releasing them in strict sequence order was tried and is wrong: it assumes the sequence numbers observed are contiguous, and one gap stalls the cursor for its whole timeout while later messages fill the slots and are evicted — which loses everything after the first message of a multi-line reply, e.g. `$I` returning the firmware version but never the IP/SSID line that follows it.)
* Fix: **the pendant stayed on "Syncing" until a command was sent, and status/DRO were stale.** GRBL realtime bytes (`?` status poll, `!`, `~`) are out-of-band and must bypass the outgoing line buffer, but the ESP-NOW send path only treated them that way when the buffer happened to be empty. Core 0 emits the `?` poll while Core 1 pushes line commands, so whenever a partial line was buffered the `?` was appended *into* that command — losing the status request and corrupting the command. Sending a newline-terminated command such as `$H` flushed the buffer and made it briefly work, which is what made the fault look like "homing fixes it". Realtime bytes now bypass the line buffer unconditionally.
* Fix: **settings queries asked the controller eight questions and could not read the answers.** `parse_dollar()` matches replies only against items registered in `configRequests`, but the connect-edge burst sent `$110`, `$130`-`$133` and `$23` as raw text without registering them — so those items were permanently "unknown" and their consumers silently kept compiled-in defaults (per-axis jog travel stuck at 0, jog max feed at 10000 mm/min, homing direction mask at its default). Present since the queries were made non-blocking; it affects UART as well as ESP-NOW. They now go through a registering, non-blocking `ConfigItem::request()`.
* Add: **the rest of the startup queries are verified and retried too.** Following the status/`$I` retry, the same treatment now covers the settings burst and the homing cycle/allow items — `configRequests` is emptied entry by entry as replies arrive, so a non-empty list is an exact "still unanswered" set and only outstanding queries are re-sent — and the `$A` alarm-code request, whose `awaiting_alarm` flag is already its own completeness check. All are bounded to four attempts and reset on the next reconnect, so nothing polls forever if a controller genuinely never answers.
* Fix: **the startup exchange is now verified and retried instead of fire-and-forget.** The pendant asked for status once every 4 s and fetched the controller config with a single one-shot burst, and nothing checked that either reply arrived. ESP-NOW does lose data on the boot burst — receive-path loss shows up at close range with full signal, so packets are being dropped after the radio hands them over (ring/queue overflow) rather than lost over the air — and a lost reply meant the pendant simply waited forever. That is the "stuck on Syncing until you home" fault: homing was just the next thing that happened to provoke a reply. The pendant now re-polls for status every 500 ms until a report is actually parsed, and re-requests the config (up to four times) if the `$I` reply came back incomplete — the firmware version arriving without the Mode/IP/SSID line that follows it in the same reply. Both checks stop the moment their data is in, so steady-state traffic is unchanged. The ISR-to-poll packet queue also goes back to 48 entries after being briefly cut to 24; the observed loss showed the smaller depth was genuinely overflowing.
* Fix: **nothing arrived from the controller after boot until you homed** — no status, and intermittently no firmware version, IP or SSID either. Outgoing ESP-NOW messages are discarded unless the link has reached its Connected state, but the pendant's connect edge is driven by GrblParser's receive timing, which the controller's keepalives satisfy *while the link is still synchronising*. The one-shot `$G`/`$I`/`$A` config burst could therefore be issued into that window and vanish silently, with nothing to retry it — until some later command (homing) happened to be sent after the link was up, at which point everything appeared at once. It was a race, which is why the symptom came and went between boots. The config fetch is now re-issued on the real radio link-up edge.
* Fix: **the pendant sat on "Syncing" after boot until you pressed Reset or Home.** Two things combined. FluidNC's `Channel::autoReport()` gates its periodic status report on `motionState()`, so an **idle machine emits no status reports at all** no matter what the report interval is — the `?` poll is the only source of status while the machine sits still, and homing was simply the first thing that changed state enough to force a report out. And the poll itself was sent as an out-of-band ESP-NOW realtime packet, which was not reaching the controller even though ordinary commands and their replies were flowing normally in both directions. `?` is now sent as a plain byte in the data stream — exactly how UART sends it, and FluidNC extracts realtime characters out of the incoming stream identically on both. It goes as its own message, never appended to a partial command, so it still cannot be spliced into one. The pendant also polls once on the link-up edge instead of waiting up to 4 s, and a poll issued while the link is still coming up is replayed rather than dropped.
* Fix: **a macro request that got an `error:` reply gave up silently.** The fallback chain is driven from the end of the JSON document, and an error reply produces no document — so the request simply stopped and the screen sat until its own timeout. Errors now advance the chain.
* Fix: **noticeably more screen flicker on ESP-NOW than UART.** Three separate causes:
  * The shared panel scratch was created at the *common* panel size (230x65) but several screens draw taller panels. `beginPanelSprite()` grew it on demand with `deleteSprite()` **then** `createSprite()` — a ~30 KB free-and-reallocate at runtime. With the radio up the heap is fragmented enough that this intermittently failed, and because the delete happened first a failure destroyed the working buffer and dropped **every panel on every screen** into the direct-draw path. The scratch is now sized once to the largest panel and never grown, shrunk or freed.
  * When a panel *did* fall back to direct drawing, `endPanelSprite()` still pushed the scratch over it — painting the last panel that used the scratch (typically from a different screen) straight over the one just drawn, as a rapid alternation between two unrelated panels.
  * The Connection screen's LINK panel was drawn straight to the display on every 100 ms tick, so the background wipe and the text redraw were two visible steps. It is now sprite-backed like every other panel — and because at 116 px tall it would need 53 KB of scratch (heap the radio needs, and when the allocation lost the panel silently went back to direct-drawing), it renders as two atomically-pushed horizontal bands instead of one oversized buffer.
* Fix: **a failed radio looked like a peer that was merely out of range.** `espnow_init()` has several failure paths (queue allocation, `esp_now_init`, PMK, callback registration) that left the pendant showing "Reconnecting" or "Waiting..." forever. The Connection and Pairing screens and the main-menu status panel now say **"Radio down — ESP-NOW failed to start"**. The panel scratch is also allocated *after* the radio rather than before it, so a cosmetic buffer can never starve `esp_now_init()`, and it falls back to a smaller size rather than failing outright.
* Fix: **Forget and machine-switching did nothing when the radio was down.** Both were gated on the radio being up, but the machine list lives in NVS and needs no radio — so a failed radio made the Machines screen silently read-only, which reads as a broken button rather than a broken radio. Only the radio-side teardown is gated now.
* Fix: **the FluidNC screen reported a controller firmware version it had never read.** The field defaulted to a plausible-looking `v3.7.16` and rendered in the same green as a genuine reading. It now shows `---` until a `[VER:]` report actually arrives, and unread `IP ADDRESS` / `WIFI SSID` placeholders are dimmed so they cannot be mistaken for real values either.
* Fix: **the battery icon showed "charging" on battery power alone.** Charging is inferred from the cell voltage, and the old test compared a fast and a slow moving average, tripping on 3 mV of divergence. Measured on hardware the cell hops +/-10-20 mV between samples, and a 20 mV hop produces almost exactly 3 mV of divergence — the threshold sat right on the noise floor. Raising it was not an option either: a real Li-Ion charge moves only ~0.2 mV per sample, far below the noise, so no rate-of-change threshold can separate the two. Detection now uses a **50 mV deadband on total travel** from a reference point instead: noise is bounded and can never accumulate past the band, while a real charge or discharge crosses it every 50 mV and re-baselines. An absolute "pinned near full = on charger" rule was also removed — it could not tell a full pack on the charger from a full pack resting on the bench, because by voltage they are identical.
* Hardening: incoming lines are truncated just short of GrblParser's unchecked 1024-byte line buffer, so an oversized reply is lost rather than corrupting adjacent memory; and a JSON reply that never completes releases the parser after 20 s rather than latching and leaving the pendant deaf to status reports.
* Upgrade note: a pendant upgrading from v2.1.x with WiFi selected will come up on **UART** — any stored transport the firmware no longer recognises falls back to the wired path rather than booting into a dead link with no way out.
* Size: the firmware is **212 KB smaller** (1,336,480 -> 1,119,405 bytes; 51.0% -> 42.7% of the app partition) and uses ~10 KB less RAM, with the WebSocket client, TLS stack, and Arduino WiFi/TCP layers all gone.
